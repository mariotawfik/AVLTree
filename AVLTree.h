...
public:
    ...
    AVLTree* Insert(int v); //Creates a new pointer then connects it depending on the value of char. Returns a pointer to the head of the tree.
    AVLTree *Find(int v); //Searches for v and returns the address
    AVLTree *Delete(int v); //Searches for v, returns the address then removes it from the tree structure
